...
class LocaleProvider extends StateNotifier<Locale> {
  LocaleProvider(this.ref, Locale locale) : super(locale);
  ...
  static Locale get deviceLanguage {
    ...
    Locale? locale;
    for (final mylocale in LocaleProvider.supportedLocales) {
      if (mylocale.toString() == localeString) {
        locale = mylocale;
        break;
      }
    }
  ...
  static String localeToLabel(Locale locale, [localize = false]) {
    ...
    switch (str) {
      ...
      case 'zh_CN':
        return localize ? S.of(context).languageChinese : "Chinese";
      case 'ms_MY':
        return localize ? S.of(context).languageMalay : "BM";
      default:
        return str;
    }
  }
...
final localeProvider = StateNotifierProvider<LocaleProvider, Locale>((ref) {
  ...
  final savedLocale = singleton<Storage>().getString(Storage.LANGUAGE_KEY);
  if (savedLocale != null) {
    locale = LocaleProvider.localeFromLocaleName(savedLocale);
  }

  return LocaleProvider(ref, locale);
});
